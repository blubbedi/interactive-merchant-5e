...
window.MerchantTradeApp = class MerchantTradeApp extends Application {
    ...
    async _updateFlag(updates = {}) {
        ...
        if (playerChar) {
            const oldFlag = playerChar.getFlag("interactive-merchant-5e", "tradeState") || {};
            await playerChar.setFlag("interactive-merchant-5e", "tradeState", {
                ...oldFlag,
                ...updates,
                timestamp: Date.now()
    ...
    async getData() {
        ...
        const flag = playerCharacter?.getFlag("interactive-merchant-5e", "tradeState");
    ...
    async _executeTrade() {
        ...
        await player.unsetFlag("interactive-merchant-5e", "tradeState");
        this.close(); 
...
Hooks.on('updateActor', (actor, data, options, userId) => {
    if (foundry.utils.hasProperty(data, "flags.interactive-merchant-5e.tradeState")) {
        const flagData = actor.getFlag("interactive-merchant-5e", "tradeState");
        ...
        if (game.user.isGM) {
            if (flagData.status === "cancelled") {
                actor.unsetFlag("interactive-merchant-5e", "tradeState");
